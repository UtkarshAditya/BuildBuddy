...
function TeamDetailPageContent() {
  ...
  const handleDragEnd = async (result: DropResult) => {
    ...
    // Update on server
    try {
      await teamsAPI.updateTask(teamId, taskId, { status: newStatus });
    } catch (err) {
  ...
  const handleDeleteTask = async () => {
    ...
    try {
      await teamsAPI.deleteTask(teamId, deleteTaskId);
                  ...
                  <Droppable key={column.id} droppableId={column.id}>
                    {(provided, snapshot) => (
                      <div
                        ...
                        className={`bg-white dark:bg-gray-800 rounded-lg shadow-sm border-t-4 ${
                          column.color
                          ...
                          {getTasksByStatus(column.id).map((task, index) => (
                            ...
                            >
                              {(provided, snapshot) => (
                                ...
                                >
                                  <Card
                                    className={`group hover:shadow-md transition-all cursor-move border-l-4 ${
                                      snapshot.isDragging
                                        ? "shadow-xl rotate-2"
                                        : ""
                                    }`}
        ...
        >
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Are you sure you want to delete this team? This will permanently
                delete the team, all tasks, and remove all members. This action
                cannot be undone.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
